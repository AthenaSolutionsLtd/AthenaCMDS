    ...
    if (dir) {
      if (!fs.existsSync(dir)) {
        new Logger("debug", "America/Chicago", "logs").log(
          "error",
          "CommandHandler",
          `Commands directory "${dir}" doesn't exist!`
        );
      }
        ...
        for (const key in options) {
          ...
          if (name !== lowerCase && instance.showWarns) {
            new Logger("debug", "America/Chicago", "logs").log(
              "info",
              "CommandHandler",
              `Command "${names[0]}" has an option of "${name}". All option names must be lower case for slash commands. AthenaCMDS will modify this for you.`
            );
          }
          ...
          if (lowerCase.match(/\s/g)) {
            ...
            new Logger("debug", "America/Chicago", "logs").log(
              "info",
              "CommandHandler",
              `Command "${names[0]}" has an option of "${name}" with a white space in it. It is a best practice for option names to only be one word. AthenaCMDS will modify this for you.`
            );
          }
